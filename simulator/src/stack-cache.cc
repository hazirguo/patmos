		...
		uword_t new_spill, uword_t new_top)
{
	switch (Phase)
	{
		case IDLE:
		{
			...
			std::cout << "reservesize" << Content.size();
			// need to spill some blocks?
			if(delta > 0)
			{
		...
		uword_t new_spill, uword_t new_top)
{
	...
	if (Content.size() <= size_blocks * Num_block_bytes) {
		...
		Content.resize(Content.size() - size_blocks * Num_block_bytes);
		std::cout << "freesize" << Content.size();
	}
		...
		uword_t &stack_top)
{
	...
	uword_t reserved_blocks = get_num_reserved_blocks(lazy_stack_spill, stack_top);
	uword_t lazy_reserved_blocks = get_num_reserved_blocks(stack_top, lazy_stack_spill);
	// need to spill some blocks?
	...
	if (lazy_reserved_blocks < reserved_blocks) {
		transfer_blocks = lazy_reserved_blocks;
	}

	// update the stack top pointer of the processor
	stack_spill -= (reserved_blocks - Num_blocks) * Num_block_bytes;
	...
	retval = transfer_blocks * Num_block_bytes;
	...
	if (stack_spill - stack_top > Num_blocks * Num_block_bytes) {
		num_blocks_not_spilled = (stack_spill - lazy_stack_spill) / Num_block_bytes;
